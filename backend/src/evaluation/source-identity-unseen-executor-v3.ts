...
import type { WorkOrderMainChainV3 } from "../coordinator/work-order-main-chain-v3.ts";
...
import type { SourceIdentityUnseenV2Dataset } from "./source-identity-unseen-v2-dataset.ts";
import type { SourceIdentityUnseenV3Dataset } from "./source-identity-unseen-v3-dataset.ts";
...
function score(dataset: EvaluationDataset, actualCases: ActualCase[]) {
  const byId = new Map(actualCases.map((item) => [item.case_id, item]));
  const cases = dataset.cases.map((expected: SourceIdentityUnseenCase) => {
    const actual = byId.get(expected.case_id);
    if (!actual) throw new Error(`missing source identity result ${expected.case_id}`);
    byId.delete(expected.case_id);
  ...
  if (byId.size !== 0) throw new Error("source identity evaluation returned unknown cases");
...
export async function executeSourceIdentityUnseenEvaluationV3(input: {
  dataset: EvaluationDataset;
  createMainChain(
    database: PGliteInterface,
    embedder: QueryEmbedder,
  ): Pick<WorkOrderMainChainV3, "versions" | "answerabilityJudge">;
}) {
  ...
  try {
    ...
    if (
      runtime.versions.answerabilityPrompt !== "answerability-v8-candidate-isolated" ||
      runtime.versions.sourceIdentityBinding !== "database-source-chain-v1"
    ) {
      throw new Error("source identity evaluation v3 requires the formal eighth-version main chain");
    }
