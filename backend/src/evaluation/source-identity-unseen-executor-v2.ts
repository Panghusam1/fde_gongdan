...
import type { WorkOrderMainChainV2 } from "../coordinator/work-order-main-chain-v2.ts";
...
export async function executeSourceIdentityUnseenEvaluationV2(input: {
  dataset: EvaluationDataset;
  createMainChain(
    database: PGliteInterface,
    embedder: QueryEmbedder,
  ): Pick<WorkOrderMainChainV2, "versions" | "answerabilityJudge">;
}) {
  ...
  try {
    ...
    if (
      runtime.versions.answerabilityPrompt !== "answerability-v7-source-policy" ||
      runtime.versions.sourceIdentityBinding !== "database-source-chain-v1"
    ) {
      throw new Error("source identity evaluation v2 requires the formal seventh-version main chain");
    }
