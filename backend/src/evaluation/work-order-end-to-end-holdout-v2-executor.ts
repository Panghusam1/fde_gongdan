...
export async function executeWorkOrderEndToEndHoldoutV2(input: {
  dataset: WorkOrderEndToEndHoldoutV2;
  embedder: QueryEmbedder;
  judge: QwenAnswerabilityJudge;
  coordinatorModel: WorkOrderCoordinatorModel;
  ...
  if (
    ...
    input.judge.modelId !== input.dataset.strategy.judge_model_id ||
    input.judge.promptVersion !== input.dataset.strategy.judge_prompt_version ||
    input.coordinatorModel.modelId !==
      input.dataset.strategy.coordinator_model_id ||
    input.coordinatorModel.promptVersion !==
  ...
  try {
    ...
    return {
      ...
      report: scoreWorkOrderEndToEndHoldoutV2(input.dataset, actualCases, {
        judge_model_id: input.judge.modelId,
        coordinator_model_id: input.coordinatorModel.modelId,
      }),
    };
  } finally {
