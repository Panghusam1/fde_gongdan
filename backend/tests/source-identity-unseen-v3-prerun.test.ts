...
test("R310：第三批未见题和第八版正式主链必须在首次模型运行前完成散列封存", async () => {
  const record = JSON.parse(
    await readFile("reports/source-identity-unseen-v3-prerun.json", "utf8"),
  ) as {
    status: string;
  ...
  for (const item of record.frozen_inputs) {
    ...
    const actual = createHash("sha256").update(await readFile(item.path)).digest("hex");
