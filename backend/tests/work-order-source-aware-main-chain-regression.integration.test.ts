...
const frozenPaths = [
  ...
  "tests/work-order-source-aware-main-chain-regression.integration.test.ts",
] as const;
...
test(
  "R291：正式第六版主链必须在十二条已暴露工单上完成真实模型升级回归",
  {
    skip: process.env.RUN_QWEN_SOURCE_AWARE_MAIN_CHAIN_REGRESSION !== "1",
    timeout: 900_000,
  },
  async () => {
    const [sourceDataset, preRunRaw] = await Promise.all([
      ...
      readFile(
        "reports/work-order-source-aware-main-chain-regression-prerun.json",
        "utf8",
      ),
    ]);
    ...
    await writeReportOnce(
      "reports/qwen-work-order-source-aware-main-chain-regression-first-run.json",
